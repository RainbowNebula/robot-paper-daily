...
def get_recent_data_dates(date_papers: Dict, limit: int = 5) -> List[str]:
    """获取 JSON 中最近 N 个实际有论文数据的日期"""
    dates = [
...
def parse_score_from_summary(summary: str) -> int:
    """当历史 llm_score 无效时，从 llm_summary 中重新解析评分"""
    if not summary:
        return 0

    match = LLM_SCORE_PATTERN.search(str(summary))
    if not match:
        return 0
...
def get_effective_score(paper: Dict) -> int:
    """优先使用 llm_score；无效时从 llm_summary 回退解析"""
    ...
    return parse_score_from_summary(paper.get("llm_summary", ""))
...
def generate_paper_row(paper: Dict) -> str:
    ...
    # 修复：历史数据里 llm_score=0 时，从 summary 回退解析
...
def generate_date_section(date: str, papers: List[Dict], is_latest: bool, anchor_id: str) -> str:
    ...
    # 修复：排序也使用回退后的有效评分
    sorted_papers = sorted(papers, key=get_effective_score, reverse=True)
...
def json_to_html(json_path: str, output_path: str, template_path: str) -> bool:
    ...
    # 3. 筛选最近几天有数据的日期
    # 修复：按 JSON 中实际有数据的日期取最近 5 个，避免周末少显示
    valid_dates = get_recent_data_dates(date_papers, DEFAULT_RECENT_DAYS)

    if not valid_dates:
        logging.warning(f"⚠️ 最近 {DEFAULT_RECENT_DAYS} 个数据日期内无有效论文数据")
        valid_dates = []

    latest_date = valid_dates[0] if valid_dates else None
    total_papers = sum(len(date_papers[d]) for d in valid_dates)

    logging.info(f"✓ 将生成包含 {total_papers} 篇论文的 HTML 页面（最近{len(valid_dates)}个数据日期）")
